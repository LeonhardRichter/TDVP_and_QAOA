    ...
    def get_qaoa_gram(self, delta: tuple) -> NDArray:
        ...
        if self.gram_mode == "single":
            ...
            def A(left: tuple, right: tuple, delta):
                ...
                """
                right_state = U_i(delta, *right).run(
                    tensor([minus for _ in range(self.n)])
                )
                left_state = U_i(delta, *left).run(
                    tensor([minus for _ in range(self.n)])
                )
...
def psi(pars: tuple[float]) -> Qobj:
    return tensor(
        [
            np.cos(pars[0] / 2) * basis(2, 0)
            + np.exp(1j * pars[1]) * np.sin(pars[0] / 2) * basis(2, 1)
            for _ in range(2)
        ]
    )


tdvp_ = tdvp_optimizer(
    psi, tensor(sigmaz(), sigmaz()), np.array([[1, 2], [2, 1]]), gram_mode="double"
)
t_0 = time()
